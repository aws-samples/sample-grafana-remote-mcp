...
export class SecurityStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: SecurityStackProps) {
    ...
    // Add domain for hosted UI (needed for 3-legged OAuth flow)
    ...
    // Create separate app client for client credentials flow (machine-to-machine)
    this.appClientClientCredentials = this.userPool.addClient("mcp-client-credentials", {
      userPoolClientName: "mcp-client-credentials",
      generateSecret: true,
      oAuth: {
        flows: {
          clientCredentials: true, // Only client credentials flow
        },
        scopes: [
          cognito.OAuthScope.resourceServer(resourceServer, {
            scopeName: "read",
    ...
    new cdk.CfnOutput(this, "UserPoolClientId", {
      value: this.appClientClientCredentials.userPoolClientId,
      description: "The Client ID for the Cognito User Pool Client (Client Credentials)",
    });
    ...
    // Store the app client ID in SSM for later use in MCP server stack (use client credentials client)
    new ssm.StringParameter(this, "UserPoolClientIdParameter", {
      parameterName: `/mcp/cognito/user-pool-client-id-${props.resourceSuffix}`,
      description: "The user pool client ID for client credentials authentication",
      stringValue: this.appClientClientCredentials.userPoolClientId,
    });

    // Store the client secret in Secrets Manager for secure access by MCP server
    const clientSecret = new secretsmanager.Secret(this, "UserPoolClientSecret", {
      secretName: `/mcp/cognito/user-pool-client-secret-${props.resourceSuffix}`,
      description: "The user pool client secret for OAuth authentication",
      secretStringValue: this.appClientClientCredentials.userPoolClientSecret,
    });

    // Suppress rotation warning for OAuth client secret
    NagSuppressions.addResourceSuppressions(clientSecret, [
      {
        id: "AwsSolutions-SMG4",
        reason: "OAuth client secrets are long-lived and don't require automatic rotation. Manual rotation would invalidate existing access tokens and require coordination with all clients.",
      },
    ]);
